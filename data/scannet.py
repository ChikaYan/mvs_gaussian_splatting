...
class ScannetDepthMVS(Dataset):
    def __init__(self, root_dir, split, n_views=3, levels=1, img_wh=(640, 480), downSample=1.0, max_len=-1):
        """
        ...
        split_file = "/home/hl589/mvsnerf-main/splits/ScanNet_%d_frame_%s.npy" % (self.n_views, self.split)
        print(split_file)
        data = np.load(split_file, allow_pickle=True).tolist()
        self.samples = []
        for item in data:
            scene = item.get('scene', '')
            scene_folder_path = os.path.join(self.root_dir, scene)
            if os.path.exists(scene_folder_path):
                # print(scene_folder_path)
                self.samples.append(item)

        print(f"loading {len(self.samples)} for {split}")
    ...
    def parse(self, index):
        ...
        scene = sample['scene']
        frame_ids = []
        assert (len(sample['refs']) == (self.n_views - 1))
        frame_ids.extend(sample['refs'])
        frame_ids.extend(sample['refs'])
        frame_ids.extend([sample['target']])
        return scene, frame_ids

    def get_pose(self, scene, frame_id):
        path = os.path.join(self.root_dir, scene, 'pose',
                            "%06d.txt" % frame_id)
    ...
    def __getitem__(self, idx):
        ...
        scene, views = self.parse(index)
        src_views = views[:-1]
        scene_path = os.path.join(self.root_dir, scene)
        target_view = views[-1]
        view_ids = [views[i] for i in range(3)]+[target_view]
        # print(view_ids)
        

        f=open(os.path.join(scene_path,'nearfar.txt'))
        line=f.readlines()
        near_depth, far_depth=line[-1].split(',')
        near_depth = float(near_depth)*1000*self.scale_factor
        far_depth = float(far_depth)*1000*self.scale_factor
        f.close()
        ...
        for i, vid in enumerate(view_ids):
            img_filename = os.path.join(self.root_dir, scene, 'rgb', "%06d.jpg" % vid)
            depth_filename = os.path.join(self.root_dir, scene, 'depth', "%06d.png" % vid)
            ...
            if os.path.exists(depth_filename):  # and i == 0
                depth_h = np.array(cv2.imread(depth_filename, 2),dtype=np.float32)
                # depth_h = cv2.resize(depth_h, None, fx=self.downSample, fy=self.downSample,
                #              interpolation=cv2.INTER_NEAREST)  # !!!!!!!!!!!!!!!!!!!!!!!!!
                # depth_h = cv2.resize(depth_h, None, fx=1.0 / 4, fy=1.0 / 4,
                #                 interpolation=cv2.INTER_NEAREST)  # !!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            extrinsic = self.get_pose(scene,vid).astype('float32')
            extrinsic[:3, 3] *= self.scale_factor
            proj_mat_l = np.eye(4)
            proj_mat_l[:3, :4] = intrinsic[:3,:3] @ extrinsic[:3, :4]
            proj_mat_ls = proj_mat_l
            near_far = [near_depth,far_depth]
        ...
        c2ws_all = []
        for vid in view_ids_all:
            extrinsic = self.get_pose(scene,vid).astype('float32')
            extrinsic[:3, 3] *= self.scale_factor
            c2ws_all.append(np.linalg.inv(extrinsic))
        c2ws_all = np.stack(c2ws_all)
        # c2ws_all = self.cam2worlds[self.remap[view_ids_all]]

        sample['images'] = imgs  # (V, H, W, 3) V=4
