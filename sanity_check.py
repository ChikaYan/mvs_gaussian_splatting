...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        num_points  = self.train_dataset.init_pointclouds.shape[0]
        self.features = nn.Parameter(torch.randn(num_points,16,3).to('cuda').contiguous().requires_grad_(True))
        self._scaling = nn.Parameter(torch.randn(num_points,3).to('cuda').contiguous().requires_grad_(True))
        self._rotation = nn.Parameter(torch.randn(num_points,4).to('cuda').contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.randn(num_points,1).to('cuda').contiguous().requires_grad_(True))
        self.grad_vars = []
        self.grad_vars = [
            {'params': [self.features], 'lr': args.lrate, "name": "feature"},
            {'params': [self._opacity], 'lr': args.lrate, "name": "opacity"},
            {'params': [self._scaling], 'lr': args.lrate, "name": "scaling"},
            {'params': [self._rotation], 'lr': args.lrate, "name": "rotation"},
        ]   
        self.init_volume()
        # if args.dataset_name=='dtu_ft_gs':
        #     self.grad_vars += {'params': self.volume.parameters(), 'lr': args.volume_lr, "name": "volume"}
        # else:
    ...
    def training_step(self, batch, batch_nb):
        ...
        scales = self.scaling_activation(self._scaling)*0.05
        ...
        rendered_image, radii = rasterizer(
            ...
            cov3D_precomp = None)
        if self.idx%500==0:
            torchvision.utils.save_image(rendered_image, f'runs_fine_tuning/{self.args.expname}/train_{self.idx:05d}' + ".png")
            torchvision.utils.save_image(rgbs_target, f'runs_fine_tuning/{self.args.expname}/traingt_{self.idx:05d}' + ".png")
        ...
        # pdb.set_trace()
        if False:
            path = f'runs_fine_tuning/{self.args.expname}/point_cloud.ply'
