...
class MVSSystem(LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        ##################  rendering #####################
        keys = ['val_psnr', 'val_depth_loss_r', 'val_abs_err', 'mask_sum','val_ssim','val_lpips','val_depth_rmse','val_depth_abs_rel'] + [f'val_acc_{i}mm' for i in self.eval_metric]
        ...
        with torch.no_grad():
            ...
            lpips_i = lpips_vgg(imgs[0,-1][None, ...].to(device).contiguous(),rgb[None, ...].to(device).contiguous(), normalize=True)#.item()
            ssim_i = ssim(imgs[0,-1][None, ...].to(device).contiguous(), rgb[None, ...].to(device).contiguous(), 11, True)#.item()
            ...
            if self.args.with_depth:
                ...
                log['val_depth_rmse'] = rmse
                abs_rel = torch.abs(depth_r[mask] - depth_gt_render[mask]) / depth_gt_render[mask]
                abs_rel = (abs_rel.sum()/mask.sum())#.item()
                log['val_depth_abs_rel'] = abs_rel
